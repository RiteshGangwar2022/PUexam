    ...
    try {
        ...
        if (admindata) {
            ...
            if (!ismatch) {
             ...
             res.status(200).json({ message: "user singin successfully" });
...
const Signup = async(req,res)=>{
    ...
    try {
            const { name, email,age, mobile,gender, password} = req.body;
                 
            const hash= await hashPassword(req.body.password);
            const newadmin = new Admin({ name, email,age, mobile,gender, password:hash});
